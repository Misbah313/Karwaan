...
// Delete profile picture
  ...
  // create workspace
  Future<Workspace> createWorkspace(String workspaceName, String workspaceDec,
    ...
    try {
      ...
      final workspace = await client.workspace.createWorkspace(
          workspaceName, workspaceDec, token,
          backgroundColor: backgroundColor, isPrivate: isPrivate);
  ...
  Future<Workspace> updateWorkspace(
      String newName, String newDec, String newColor, int workspaceId) async {
    try {
      final token = await _authTokenStorage.getToken();
      if (token == null) {
        throw Exception('Please login before preforming this action.');
      ...
      final update = await client.workspace.updateWorkspace(workspaceId, token,
          newName: newName, newDes: newDec, newColor: newColor);
  ...
  Future<List<BoardCard>> getAllUserCards() async {
    try {
      ...
      final cards = await client.boardCard.getAllUserCards(token);
      return cards;
    } catch (e) {
      rethrow;
    }
